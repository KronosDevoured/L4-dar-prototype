"""
Dominus Center of Mass Multi-Axis Measurement Script
...
class DominusCenterOfMassMultiAxis(BaseAgent):
    def __init__(self, name, team, index):
        ...
        self.stabilize_duration = 3.0  # Wait 3 seconds for perfect stabilization
        ...
        # Dominus hitbox specs
        self.car_length = 127.93  # Dominus hitbox length in uu
        self.nose_offset = self.car_length / 2

        # Output directory
        self.output_dir = Path("C:/Users/itsju/AppData/Roaming/bakkesmod/bakkesmod/data/dominus_com_test")
        self.output_dir.mkdir(parents=True, exist_ok=True)

    def initialize_agent(self):
        """Called once when bot starts"""
        self.logger.info("Dominus Multi-Axis Center of Mass Measurement Bot initialized")
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        # Phase 1: Wait for match to start
        if not self.match_started:
            if packet.game_info.is_match_ended or not packet.game_info.is_round_active:
                return controller
            self.match_started = True
            self.logger.info("Match started - waiting for spawn")
            return controller

        # Phase 2: Teleport to test position
        if not self.teleported:
            self.teleport_to_center()
            self.teleported = True
            self.stabilizing = True
            self.stabilize_start_time = time.time()
            self.logger.info(f"Teleported - Stabilizing for {self.stabilize_duration}s before {self.current_axis} test")
            return controller

        # Phase 3: Stabilize and wait for perfect leveling
        if self.stabilizing:
            elapsed = time.time() - self.stabilize_start_time

            # Apply leveling input during stabilization
            pitch = car.physics.rotation.pitch
            roll = car.physics.rotation.roll
            controller.pitch = -pitch * 5.0
            controller.roll = -roll * 5.0
            controller.pitch = max(-1.0, min(1.0, controller.pitch))
            controller.roll = max(-1.0, min(1.0, controller.roll))

            # After stabilization time, check if level and start test
            if elapsed >= self.stabilize_duration:
                if abs(pitch) < 0.05 and abs(roll) < 0.05:
                    self.stabilizing = False
                    self.test_start_time = time.time()
                    self.logger.info(f"Stabilization complete - Starting {self.current_axis} rotation test")
                else:
                    # Not level yet, extend stabilization
                    self.stabilize_start_time = time.time()
                    self.logger.info(f"Not level yet (pitch={pitch:.3f}, roll={roll:.3f}) - Extending stabilization")

            return controller

        # Phase 4: Run the rotation test
    ...
    def analyze_and_save(self):
        ...
        for axis in ['pitch', 'yaw', 'roll']:
            csv_file = self.output_dir / f"dominus_com_{axis}_data.csv"
        ...
        # Save analysis
        analysis_file = self.output_dir / "dominus_com_multiaxis_analysis.txt"
        with open(analysis_file, 'w') as f:
            f.write("DOMINUS CENTER OF MASS MULTI-AXIS ANALYSIS\n")
            f.write("=" * 60 + "\n\n")
            ...
            f.write("COMPARISON TO KNOWN DATA:\n")
            f.write("COMPARISON TO CURRENT L4 VALUES:\n")
            f.write(f"  Current L4 xOffset: -14.0 uu\n")
            f.write(f"  Measured xOffset: {-offset_x:.2f} uu\n")
            f.write(f"  Current L4 yOffset: +15.0 uu\n")
            f.write(f"  Measured zOffset: {offset_z:.2f} uu\n\n")
        ...
        # Print to console
        print("\n" + "=" * 60)
        print("DOMINUS CENTER OF MASS MULTI-AXIS MEASUREMENT COMPLETE")
        print("=" * 60)
